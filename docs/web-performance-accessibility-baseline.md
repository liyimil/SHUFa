...
- axe-core 在桌面 Chrome 与 Pixel 7 视口检查首页、未筛选/已筛选单字结果、范字详情、有效分享和撤销分享六种状态。
...
| ---------- | ---: | -------: | -------: | --: | ------ | -------- | ----- | ----- | --: |
| 首页       |   99 |      100 |      100 | 100 | 768 ms | 1,943 ms | 6 ms  | 88 ms |   0 |
| “永”单字页 |   99 |      100 |      100 | 100 | 837 ms | 1,916 ms | 19 ms | 65 ms |   0 |

两页均达到当前预算，单字页 LCP 低于 PRD 的 3 秒目标。axe 的两种视口、六种页面状态没有发现自动可检测违规。审计时还补齐了站点图标，消除了 `/favicon.ico` 404 造成的浏览器控制台错误。
