...
| `brace-expansion`   | 2.1.2/5.0.7 | 5.0.8    | ESLint、Expo 与 OpenAPI 工具链；修复无界展开导致的内存耗尽拒绝服务    |
| `js-yaml`           | 5.2.1       | 5.2.2    | Nest Swagger；修复别名密集输入导致的拒绝服务                          |
| `valibot`           | 1.2.0       | 1.4.2    | Prisma 开发工具；修复正则校验导致的拒绝服务                           |
| `next`              | 16.2.10     | 16.2.11  | Web 与 Admin 直接依赖；修复上游公布的中高危服务端公告                 |
...
- `GHSA-pm4m-ph32-ghv5`（js-yaml 5.x）
- `GHSA-5qjj-4xww-7phc`（valibot）
- `GHSA-89xv-2m56-2m9x`、`GHSA-68g3-v927-f742`、`GHSA-4633-3j49-mh5q`、`GHSA-4c39-4ccg-62r3`、`GHSA-p9j2-gv94-2wf4`、`GHSA-q8wf-6r8g-63ch`、`GHSA-955p-x3mx-jcvp`、`GHSA-6gpp-xcg3-4w24`、`GHSA-m99w-x7hq-7vfj`（Next.js 16.2.11 修复组）
...
- 2026-07-26 定向回归：Web 14 项测试及 Next 16.2.11 生产构建、Admin 11 项测试及生产构建、Mobile 35 项测试及 Android 导出全部通过；`pnpm security:audit` 再次为零。
