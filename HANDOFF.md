...
5. 当前功能分支为 `codex/openapi-client-errors`，实现提交 `3fc147d` 已推送并建立草稿 PR #1；继续开发应保留该分支上的契约、错误处理和离线草稿改动，不要退回旧 `main`。
...
- axe-core 在两种视口覆盖首页、App 引导、未筛选/已筛选单字页、详情、有效/撤销分享七种状态；Lighthouse 13 对 production standalone 的首页和单字页各运行三次并按中位数执行 3 秒首屏及 TTFB/TBT/CLS 预算，方法和基线见 `docs/web-performance-accessibility-baseline.md`。
- 首页和静态 `/app` 引导页明确区分 Web 的公开查字/来源核验与 App 的拍摄、对比、建议、复练和记录能力；真实内测渠道没有验收前只显示“内测准备中”，不伪造下载按钮或商店链接。
